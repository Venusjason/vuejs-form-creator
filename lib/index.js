      ...
      return h("div", [h("button", {
        ...
        "on": {
          "click": function click() {
            return _this.showVal = !_this.showVal;
          }
        }
      }, [this.showVal ? '关闭' : '表单值']), this.showVal && h("div", {
        "style": dialogStyle
      }, [h("pre", [JSON.stringify(this.model, null, 2)])])]);
    }
  };
    ...
    return {
      ...
      render: function render(h) {
        ...
        var _vm$option = vm.option,
            ...
            rest = _objectWithoutProperties(_vm$option, ["status", "debug"]);
        ...
        var elForm = h(UI.Form, {
          ...
          ref: 'form'
        }, [].concat(_toConsumableArray(vm.$slots.prepend || []), _toConsumableArray(vm.renderFormItems(h) || []), _toConsumableArray(vm.$slots.append || [])));
        return h('div', {}, [elForm, // 开发环境 提供表单值查看功能
        debug && h(formMsg, {
          props: {
            model: vm.formValues
          }
      ...
      methods: {
        ...
        renderFormItem: function renderFormItem(h, _ref) {
          var tag = _ref.tag,
              ...
              detail = _objectWithoutProperties(_ref, ["tag", "component", "rules", "previewFormItemValue", "label", "item", "class", "style"]);
        ...
        renderFormItems: function renderFormItems(h) {
          ...
          var mapFormItems = function mapFormItems(item, i) {
            // 递归调用
            if (Array.isArray(item) && item.length > 0) {
              return item.map(function (itemChild, j) {
                return mapFormItems(itemChild, j);
              });
            }

            var defaultSpan = vm.option.itemSpan || 24;
            var formItem;
          ...
          return this.fields.map(function (item, index) {
            ...
            if (Array.isArray(item) && item.length > 0) {
              var _formItem = item.map(function (itemChild, i) {
                return mapFormItems(itemChild, i);
              });
            ...
            var formItem = mapFormItems(item);
            return h(UI.Row, {
              // 提升渲染准确性
              key: index
            }, [formItem]);
